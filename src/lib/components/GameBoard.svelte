<script lang="ts">
  ...
  $: myCaptured = isPlayer1
    ? gameState?.player1Captured?.length || 0
    : gameState?.player2Captured?.length || 0;
  $: opponentCaptured = isPlayer1
    ? gameState?.player2Captured?.length || 0
    : gameState?.player1Captured?.length || 0;
  $: myCapturedCards = isPlayer1
    ? gameState?.player1Captured || []
    : gameState?.player2Captured || [];
  $: opponentCapturedCards = isPlayer1
    ? gameState?.player2Captured || []
    : gameState?.player1Captured || [];
  ...
  function calculateDragBuildValues(): number[] {
    ...
    for (let v = 2; v <= 14; v++) {
      ...
      const hasCapturingCard = myHand.some(
        (c) => c.id !== dragBuildCard?.id && getCardValues(c).includes(v)
      );
...
<div class="game-board">
  ...
  <div class="opponent-area">
    ...
    <div class="hand opponent-hand">
      {#each opponentHand as _, i}<div class="card-back">
    ...
    <CapturedPile
      ...
      position="top"
      isSelectable={isMyTurn && !!selectedCard}
  ...
  <div class="table-area">
          ...
          {#each builds as build}
            ...
            >
              ...
              <div class="build-cards">
                {#each build.cards as card}<Card {card} size="small" isPlayable={false} />{/each}
              </div>
            </button>
          {/each}
        </div>
    ...
    {#if selectedTableCards.length > 0 || selectedBuildIds.length > 0}
      <div class="selection-summary">
        ...
        {#if selectedTableCards.length > 0}<span class="selection-count"
            >{selectedTableCards.length} card{selectedTableCards.length > 1 ? 's' : ''}</span
          >{/if}
        {#if selectedBuildIds.length > 0}<span class="selection-count"
      ...
      {#each myHand as card}
        ...
        >
          <Card
            {card}
            size="medium"
            isPlayable={isMyTurn && !isProcessing}
    ...
    {#if selectedCard && isMyTurn}
      <div class="action-buttons">
        <p class="selected-info">
          Playing: <strong>{selectedCard.rank} of {selectedCard.suit}</strong>
          {#if selectedTableCards.length > 0}<span class="with-cards"
  ...
  <div class="game-finished-overlay">
    <div class="game-finished-content">
      ...
      <div class="final-scores">
        <div class="score-row">
          <span class="score-name">{players[0]?.name || 'Player 1'}</span><span class="score-value"
            >{gameState?.player1Score || 0}</span
          >
        </div>
        <div class="score-divider">vs</div>
        <div class="score-row">
          <span class="score-name">{players[1]?.name || 'Player 2'}</span><span class="score-value"
            >{gameState?.player2Score || 0}</span
          >
        </div>
      </div>
...
<style>
  ...
  .hand-card.playable:hover .table-area {
  ...
  .table-card-btn.selectable:hover {
  ...
  .build-value-selector label {
    color: var(--text-primary);
    font-weight: 600;
  }
  .build-value-selector select {
    padding: 0.5rem 1rem;
    border-radius: 0.5rem;
    border: 2px solid var(--casino-gold);
    background: rgba(0, 0, 0, 0.3);
  ...
  .action-menu-btn.capture {
    background: linear-gradient(135deg, #10b981 0%, #059669 100%);
  }
  .action-menu-btn.build {
    background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%);
  }
  .action-menu-btn.trail {
    background: linear-gradient(135deg, #6b7280 0%, #4b5563 100%);
  }
