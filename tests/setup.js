...
vi.mock('convex/react', async () => {
    ...
    return {
        ...actual,
        ConvexProvider: ({ children }) => React.createElement(React.Fragment, null, children),
        useMutation: (fn) => {
            // Mock createRoom mutation
            if (fn && fn.toString().includes('createRoom')) {
                return async ({ playerName }) => {
            ...
            if (fn && fn.toString().includes('joinRoom')) {
                return async ({ roomId, playerName }) => {
...
globalThis.testUtils = {
    ...
    mockFetchError: (error) => {
        globalThis.fetch.mockRejectedValueOnce(new Error(error));
    }
};
