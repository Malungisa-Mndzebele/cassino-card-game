...
vi.mock('../apiClient', async () => {
  return {
    api: {
      ...
      setPlayerReady: {
        setPlayerReady: vi.fn().mockResolvedValue({
          success: true,
...
describe('Simple Game Flow Tests', () => {
  ...
  describe('App Rendering', () => {
    it('should render the app with all main components', async () => {
      ...
      expect(screen.getAllByTestId('game-settings')).toHaveLength(2); // Both real and mock components
    });

    it('should show room creation form by default', () => {
      renderWithProviders(<App />);
