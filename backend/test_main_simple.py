...
class TestCasinoAPI:
    ...
    def test_create_room(self):
        """Test room creation"""
        response = app.post("/rooms/create", json={
    ...
    def test_join_room(self):
        ...
        # Then join it
        response = app.post("/rooms/join", json={
    ...
    def test_get_game_state(self):
        ...
        # Then get state
        response = app.get(f"/rooms/{self.room_id}/state")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_set_player_ready(self):
        ...
        # Set player1 ready
        response = app.post("/rooms/player-ready", json={
            "room_id": self.room_id,
    ...
    def test_start_shuffle(self):
        ...
        # Start shuffle
        response = app.post("/game/start-shuffle", json={
            "room_id": self.room_id,
            "player_id": self.player1_id
        })
        assert response.status_code == 200
    ...
    def test_select_face_up_cards(self):
        ...
        response = app.post("/game/select-face-up-cards", json={
            "room_id": self.room_id,
            "player_id": self.player1_id,
            "card_ids": []
        })
    ...
    def test_play_card_trail(self):
        ...
        # Play the card (trail)
        response = app.post("/game/play-card", json={
            "room_id": self.room_id,
            "player_id": self.player1_id,
    ...
    def test_reset_game(self):
        ...
        # Reset game
        response = app.post("/game/reset", params={"room_id": self.room_id})
        assert response.status_code == 200
        data = response.json()
        assert data["success"] == True
