...
export const playCard = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const playerHand = args.playerId === 1 ? gameState.player1Hand : gameState.player2Hand;
        const cardIndex = playerHand.findIndex((card) => card.id === args.cardId);
        if (cardIndex === -1) {
        ...
        if (args.action === 'capture') {
            ...
            const capturedPile = args.playerId === 1 ? gameState.player1Captured : gameState.player2Captured;
            ...
            const capturingCards = playerHand.filter((card) => getCardValue(card.rank) === args.buildValue && card.id !== args.cardId);
            if (capturingCards.length === 0) {
                const availableValues = playerHand
                    .filter((card) => card.id !== args.cardId)
