...
export const selectFaceUpCards = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (args.playerId !== 1)
            throw new Error('Only Player 1 can select face-up cards');
        if (gameState.phase !== 'cardSelection')
            throw new Error('Cannot select cards at this time');
        if (!args.cardIds || args.cardIds.length !== 4)
            throw new Error('Must select exactly 4 cards');
        ...
        for (const cardId of args.cardIds) {
            ...
            if (!card)
                throw new Error('Card extraction failed');
